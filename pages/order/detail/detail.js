...
Page({
  ...
  takeDo(msg) {
    ...
    app.com.post('help/jd', {
      ...
      console.log('cr45mes25',wx.getStorageSync("user").id);
      wx.hideLoading()
      if (res.code == 1) {
        wx.showToast({
          title: '接单成功',
  ...
  onLoad: function (options) {
    console.log('cr45mes24',wx.getStorageSync("user"),this.data.wx_id===this.data.wx_id);
    _this = this
    this.setData({
      id:options.id
    })
  ...
  makePhoneCall(e){
    ...
    console.log('cr45mes23',this.data.wx_id,this.data.list.wx_id,this.data.list.jd_id);
